...
\subsection{Gruppen-Detail-Ansicht}
    ...
    \subsection{Rezept-Erstellungs-Ansicht}
        ...
        \subsubsection*{IngredientWidget}
            Die Klasse beschreibt eine Widget dass eine Zutat darstellt.
            \paragraph*{Attribut}
                \subparagraph*{ingredient: Ingredient} - Die darzustellende Zutat.

            \paragraph*{Methoden}
                \subparagraph*{delete()} - Das IngredientWidget wird nicht mehr in der Rezept-Erstellungs-Ansicht angezeigt und ist die Zutat ist nicht mehr teil des Rezepts.
...
\subsection{Verwaltungsansicht}
    ...
    \subsubsection{SettingsPageState}
        ...
        \paragraph*{Methoden}
            \subparagraph*{setProfileImage(image: Image)}
            \subparagraph*{removeProfileImage()}
            \subparagraph*{setUserName(value: string)}
            \subparagraph*{joinGroup()}
            \subparagraph*{leaveGroup(id: string)}
            \subparagraph*{groupeDetail()}
            \subparagraph*{toggleFavourite(id: string)}
            \subparagraph*{reportRecipe(id: string)}
            \subparagraph*{createRecipe(data: Recipe)}
            \subparagraph*{exportRecipe(id: string)}
            \subparagraph*{logout()}
            \subparagraph*{deleteAccount()}
            \subparagraph*{deleteRecipe(id: string)}
