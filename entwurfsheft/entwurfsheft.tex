...
Es gibt verschiedene Arten von Providern in Riverpod, wie z.B. \texttt{Provider}, \texttt{ChangeNotifierProvider} und \texttt{FutureProvider}. Jeder Provider hat eine spezifische Funktionalität, abhängig von den Anforderungen der Anwendung.
...
Der \texttt{ChangeNotifierProvider} basiert auf dem Konzept des \Gls{ChangeNotifier} aus dem Flutter-Framework und ermöglicht das Verwalten und Aktualisieren von Zuständen in einer Flutter-App. Es stellt einen \Gls{ChangeNotifier} bereit, der Änderungen an seinen Abonnenten meldet.

Der \texttt{FutureProvider} ist speziell für den Umgang mit asynchronen Operationen geeignet. Er stellt einen Wert bereit, der auf eine zukünftige Antwort wartet, z.B. eine Datenabfrage von einer API.
...
\subsection{Verwendete Provider}
In der App werden die folgenden Provider benutzt.
\paragraph{\texttt{userRepositoryProvider: Provider<UserRepository>}} Stellt ein \texttt{UserRepository}-Objekt bereit, das für die Kommunikation mit dem Server für die Verwaltung der Benutzerdaten und des Tokens zuständig ist.
\paragraph{\texttt{userServiceProvider: Provider<UserService>}} Stellt ein \texttt{UserService}-Objekt bereit, das für die Kommunikation mit dem Server für die Verwaltung der Benutzerdaten zuständig ist.

\paragraph{\texttt{recipeRepositoryProvider: Provider<RecipeRepository>}} Stellt ein \texttt{RecipeRepository}-Objekt bereit, das für die Verwaltung der Rezeptdaten zuständig ist.
