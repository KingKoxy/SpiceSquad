...
Der \texttt{AddButton} ist ein Button, der ein Plus-Symbol enthält. Er wird verwendet, um ein neues Element hinzuzufügen. Er erbt von der Flutter-Klasse \texttt{ElevatedButton}.
\begin{figure}[htp]
    \centering
...
Der \texttt{RemoveButton} ist ein Button, der ein Kreuz-Symbol enthält. Er wird verwendet, um ein Element zu entfernen. Er erbt von der Flutter-Klasse \texttt{ElevatedButton}.
\begin{figure}[htp]
    \centering
...
\subsubsection{\texttt{CTAButton}}
Der \texttt{CTAButton} (Call-To-Action-Button) ist ein Button, der eine Aktion ausführt. Er erbt von der Flutter-Klasse \texttt{ElevatedButton}. Er zeichnet sich vor allem durch seine rote Farbe aus.
\begin{figure}[htp]
    \centering
    \includegraphics[height = 1cm]{images/presentationLayer/uiElements/ctaButton.png}
    \caption{Darstellung eines CTAButtons}
\end{figure}
\paragraph*{Konstruktor}
Der Konstruktor nimmt eine Funktion \texttt{onPressed} entgegen, die ausgeführt wird, wenn der Button gedrückt wird. Zudem kann ein String \texttt{text} übergeben werden, der den Text des Buttons bestimmt.

\subsubsection{\texttt{SimpleTextField}}
Der \texttt{SimpleTextField} ist ein Textfeld, das verwendet wird, um Text einzugeben. Er erbt von der Flutter-Klasse \texttt{TextField}.
\begin{figure}[htp]
    \centering
    \includegraphics[height = 1cm]{images/presentationLayer/uiElements/simpleTextField.png}
    \caption{Darstellung eines SimpleTextFields}
\end{figure}
\paragraph*{Konstruktor} Der Konstruktor kann einen TextEditingController \texttt{controller} entgegennehmen, der verwendet wird, um den Text zu verwalten. Zudem kann ein String \texttt{hintText} übergeben werden, der als Platzhalter im Textfeld dient. Eine Funktion \texttt{onChanged} kann übergeben werden, die ausgeführt wird, wenn sich der Text im Textfeld ändert. Zuletzt kann ein Boolean \texttt{obscuredText} übergeben werden, der bestimmt, ob der Text im Textfeld versteckt wird. Dies wird beispielsweise bei Passwörtern verwendet.
...
\begin{figure}[htp]
    \centering
    \includegraphics[height = 1cm]{images/presentationLayer/uiElements/navBar.png}
    \caption{Darstellung der NavBar}
\end{figure}
\newpage

\subsection{\texttt{LoginScreen}}
Die Klasse bildet die Loginansicht ab. Sie verwendet die Klassen \texttt{SimpleTextField} und \texttt{CTABut\-ton} aus dem Ordner \texttt{commonWidgets}. Die Klasse \texttt{LoginScreen} ist ein \texttt{StatelessWidget}, da sie keinen eigenen Zustand besitzt.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
\subsubsection*{Attribute}
\paragraph{emailController: TextEditingController} 
Der Controller, die die Eingabe der E-Mail-Adresse des Nutzers verwaltet. Über ihn kann der Wert des Eingabefelds abgerufen werden oder eine Fehlermeldung angezeigt werden.
\paragraph{passwordController: TextEditingController}
Der Controller, die die Eingabe des Passworts des Nutzers verwaltet. Über ihn kann der Wert des Eingabefelds abgerufen werden oder eine Fehlermeldung angezeigt werden.
\subsubsection*{Methoden}
\paragraph{login(): Future<void>}
Leitet eine Loginanfrage über einen Provider an den zuständigen Service weiter. Im Anschluss werden, je nachdem ob die Anfrage erfolgreich war oder nicht, Fehlermeldungen an den entsprechenden Eingabefeldern angezeigt, oder der Nutzer zur Hauptansicht weitergeleitet.
