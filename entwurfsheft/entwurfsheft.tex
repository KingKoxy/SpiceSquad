...
Es wird eine Anfrage zum Erhalten des angemeldeten Nutzers an den Server gestellt. Gibt ein Future-Objekt zurück, das aufgelöst wird, wenn der Nutzer erhalten wurde. Ist der Nutzer nicht angemeldet wird null zurückgegeben.
...
\subsubsection*{Methoden}
\paragraph{\texttt{RemoteRecipeRepository(UserRepository userRepository):RemoteRecipeRepository\\}} % TODO Fix maybe?
...
Die Klasse bildet die Passwort-Zurücksetzen-Ansicht ab. Sie ist ein \texttt{StatelessWidget}, da sie keinen eigenen Zustand besitzt. Sie verwendet die Klasse \hyperref[sec:backButton]{\texttt{BackButton}} aus dem \texttt{ui\_elements}-Package.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
\newpage
\subsubsection{RecipeCard}
\label{sec:recipeCard}
Die Klasse bildet eine Rezeptkarte ab. Sie ist ein \texttt{StatelessWidget}, da sie keinen eigenen Zustand besitzt. Sie verwendet die Klasse \hyperref[sec:favouriteButton]{\texttt{FavouriteButton}} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse stellt den Bildschirm dar, auf dem die Zubereitungsanweisungen eines Rezepts angezeigt werden. Sie ist ein \texttt{StatefulWidget}, da sie den Zustand \texttt{portionAmount} verwaltet. Die Klasse verwendet die Klassen \texttt{BackButton}, \texttt{FavouriteButton} und \texttt{PortionAmountField} aus dem Package \texttt{ui\_elements}. 
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse bildet die Rezept-Erstellungs-Ansicht ab. Sie ist ein \texttt{StatelessWidget}, da sie keinen Zustand verwaltet. Dies übernehmen die einzelnen Teilwidgets. Es werden verschiedene Widgets angezeigt, die es ermöglichen Rezeptdaten einzugeben oder abzuändern. Die Klasse verwendet die Klassen \texttt{BackButton} und \texttt{PortionAmountField} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse stellt die Liste der Zutaten dar. Sie ist ein \texttt{StatefulWidgget}, da sie den Zustand \texttt{ingredients} verwaltet. Sie erzeugt eine Listenansicht, in der für jedes Rezept ein Item gezeigt wird. Zusätzlich verwendet sie die Klasse \hyperref[sec:addButton]{\texttt{AddButton}} und \texttt{RemoveButton} aus dem Package \texttt{ui\_elements}. Drückt man auf den Knopf, wird man zur Zutaten-Erstellungs-Ansicht (\ref{sec:ingredientCreationScreen}) weitergeleitet.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
\label{sec:groupList}
Die Klasse bildet die Gruppenliste ab. Sie ist ein \texttt{StatelessWidget}, da sie keinen eigenen Zustand verwaltet. Sie zeigt eine Liste von Gruppen an, bei denen der Nutzer Mitglied ist. Die Klasse verwendet die Klassen \texttt{AddButton} und \texttt{RemoveButton} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse bildet die Rezeptliste des Nutzers ab. Sie ist ein \texttt{StatelessWidget}, da sie keinen eigenen Zustand verwaltet. Sie zeigt eine Liste von Rezepten an, die der Nutzer erstellt hat. Sie verwendet die Klassen \texttt{AddButton}, \texttt{RemoveButton} und \texttt{EyeButton} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse bildet die Gruppen-Detail-Ansicht ab. Sie ist ein \texttt{StatefulWidget}, da sie den Zustand \texttt{group} verwaltet. Sie verwendet die Klasse \texttt{BackButton} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse stellt ein Widget bereit, das die eine Liste der Gruppenmitglieder zeigt. Sie verwendet die Klasse \texttt{AddButton} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Die Klasse stellt ein Widget bereit, das die eine Liste der Rezepte in einer Gruppe zeigt. Sie verwendet die Klasse \texttt{EyeButton} aus dem Package \texttt{ui\_elements}.
\begin{figure}[htp]
    \begin{minipage}
        [t]{0.49\textwidth}
        \centering
...
Mit dieser Methode wird durch eine Abfrage an Firebase überprüft, ob das \glslink{id-token}{ID-Token} valide ist. Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das \glslink{id-token}{ID-Token} valide ist. Wenn das \glslink{id-token}{ID-Token} nicht valide ist, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Mit dieser Methode wird überprüft, ob der Nutzer den Adminstatus in einer Gruppe hat. Es wird in der Datenbank geprüft, ob der Nutzer den Adminstatus in der jeweiligen Gruppe hat. Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn der Nutzer den Adminstatus in der Gruppe hat. Wenn der Nutzer den Adminstatus in der Gruppe nicht hat, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn die Registrierung erfolgreich war. Wenn die Registrierung nicht erfolgreich war, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Bearbeiten des Rezepts erfolgreich war. Wenn das Bearbeiten des Rezepts nicht erfolgreich war, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Abrufen der Rezepte erfolgreich war. Wenn das Abrufen der Rezepte nicht erfolgreich war, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Markieren des Rezepts als Favorit erfolgreich war. Wenn das Markieren des Rezepts als Favorit nicht erfolgreich war, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Melden des Rezepts erfolgreich war. Wenn das Melden des Rezepts nicht erfolgreich war, wird eine Exception geworfen und das \texttt{Promise}-Objekt wird abgelehnt.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Bearbeiten der Gruppe erfolgreich war. Das \texttt{Promise}-Objekt wird abgelehnt und es wird Exception geworfen, wenn das Bearbeiten der Gruppe nicht erfolgreich war.
...
Die Methode ist asynchron und gibt ein \texttt{Promise}-Objekt zurück. Das \texttt{Promise}-Objekt wird aufgelöst, wenn das Verlassen der Gruppe erfolgreich war. Das \texttt{Promise}-Objekt wird abgelehnt und es wird Exception geworfen, wenn das Verlassen der Gruppe nicht erfolgreich war.
...
Bei einer Registrierung wird ein neuer Nutzer angelegt. Dazu muss der Nutzer seine Email-Adresse, ein Passwort und einen Nutzernamen angeben. Email und Passwort werden für eine Registrierung beim Firebase-Server verwendet, welcher eine eindeutige Nutzer-ID zurückgibt. Unter dieser Nutzer-ID werden die Daten des Nutzers in der Datenbank gespeichert. Bei erfolgreicher Anmeldung liefert der Firebase-Server ein \glslink{refresh-token}{Refresh-Token}{Refresh-Token} sowie ein \glslink{id-token}{ID-Token}, welche zur weiteren Authentifizierung des Nutzers benötigt werden.

\subsection{Anmeldung}
Bei einem Login wird auf ein bereits vorhandenes Profil zugegriffen. Der Nutzer übergibt seine Emailadresse und sein Passwort, welche anschließend vom Firebase-Server bestätigt bzw abgelehnt werden. Bei erfolgreicher Anmeldung liefert der Firebase-Server ein \glslink{refresh-token}{Refresh-Token}{Refresh-Token} sowie ein \glslink{id-token}{ID-Token}, welche zur weiteren Authentifizierung des Nutzers benötigt werden.
...
Im Gegensatz zum kurzlebigen \glslink{id-token}{ID-Token} bleibt das Refresh Token gültig, bis der Nutzer gelöscht bzw der Nutzer sich auf einem Gerät ausloggt. Es kann verwendet werden, um vom Firebase-Server ein neues \glslink{id-token}{ID-Token} anzufordern. Das \glslink{refresh-token}{Refresh-Token}{Refresh-Token} wird beim Login auf dem Gerät des Nutzers gespeichert und ermöglicht eine Authentifizierung ohne erneute Eingabe von Passwort und Email. 
