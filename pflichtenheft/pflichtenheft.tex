...
\begin{enumerate}[start=1,label={$\langle$\bfseries RM\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item In jeder Gruppe muss mindestens ein \Gls{administrator} sein, standardmäßig ist der Gruppengründer \Gls{administrator}.\label{rm:GroupAdmin}
    ...
    \item Der \Gls{autor} muss zu jedem seiner \Gls{rezept}e den Rezepttitel, die Zubereitungsdauer, die Zutaten mit Mengenangaben, den Schwierigkeitsgrad, die Portionsanzahl und eine Zubereitungsanweisungen angeben können.\label{rm:RecipeContents}
...
\begin{enumerate}[start=1,label={$\langle$\bfseries RS\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item Ein Administrator soll Rezepte von anderen Nutzern in der Gruppe aus- und \gls{einblenden} können.\label{rs:RecipeHiding}
...
\begin{enumerate}[start=1,label={$\langle$\bfseries RC\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item Der Nutzer kann den Administratoren Rezepte melden.\label{rc:RecipeMelden}
...
Gruppenadministratoren können in dieser Ansicht Gruppenmitglieder kicken, bannen, zum Administrator ernennen, Administratoren zu normalen Nutzern degradieren, Rezepte von Gruppenmitgliedern für die Gruppe \gls{ausblenden} oder die Gruppe auflösen.\par
...
Durch klicken auf den Zurück-Button gelangt der Nutzer wieder auf der Hauptansicht.\par


\subsection{Gruppen-Detail-Ansicht als Administrator (\autoref{fig:ActivityDiagramAdmin}):}
...
Durch klicken auf ein Rezept kann der Gruppenadministrator, eingeblendete Rezepte \gls{ausblenden} und ausgeblendete Rezepte einblenden.\newline
...
Ein Administrator kann einen anderen Administrator wieder zu einem normalen Nutzer machen.
