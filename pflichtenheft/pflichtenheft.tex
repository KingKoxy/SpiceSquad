...
\begin{enumerate}[start=1,label={$\langle$\bfseries RM\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item Der Nutzer muss sich registrieren können mit Nutzernamen, E-Mail-Adresse und einem Passwort.\label{RM11}
    \item Der Nutzer muss sich einloggen und ausloggen können.\label{RM2}
    ...
    \item In jeder Gruppe muss mindestens ein Gruppen Administrator sein.\label{RM5}
    ...
    \item Erstellte Rezepte müssen vom Ersteller separat verwaltet, geändert und gelöscht werden können.\label{RM9}
...
\begin{enumerate}[start=1,label={$\langle$\bfseries RS\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item Ein \Gls{Admin} soll weitere Gruppenmitglieder zum Administrator ernennen können und jenen auch den Administratorstatus wieder entziehen können.\label{RS3}
    \item Ein Administrator soll Nutzer aus Gruppen kicken, bannen und auch wieder entbannen können.\label{RS4}
    \item Ein Administrator soll Rezepte von anderen Nutzern in der Gruppe ausblenden  und wieder einblenden können.\label{RS5}
    ...
    \item Der Nutzer soll das Gruppenkürzel, von Gruppen, in denen er Mitglied ist, auch über einen QR-Code teilen können.\label{RS10}
    \item Der Nutzer soll seinen Nutzernamen ändern können.\label{RS11}
\end{enumerate}
...
\begin{enumerate}[start=1,label={$\langle$\bfseries RC\arabic*$\rangle$}, leftmargin = 5em, itemsep=4pt, parsep=4pt]
    ...
    \item Nur ein Administrator kann den Gruppennamen ändern.\label{RC7}
    \item Der Administrator kann die Gruppe verlassen und das frühste Mitglied der Gruppe wird automatisch neuer Administrator.\label{RC8}
    \item Der Nutzer kann ein Rezept dem Administrator melden.\label{RC9}
\end{enumerate}
...
Das Use-Case-Diagramm zeigt mittels Verbindungslinien, wie die einzelnen Use-Cases zueinanderstehen.
...
Dazu muss er seinen Nutzernamen, seine E-Mail-Adresse und ein Passwort eingeben.
...
Es können Zutaten, Zubereitungsanweisung, Schwierigkeitsgrad und Kochzeit für das neue Rezept angegeben werden.
...
Gruppen Administratoren können in dieser Ansicht Gruppenmitglieder entfernen, bannen, zum Administrator ernennen, als Administrator entfernen, Rezepte von Gruppenmitgliedern für die Gruppe ausblenden oder die Gruppe auflösen.\par
...
Der Nutzer hat die Möglichkeit die Rezepte Liste durch vorgegebene Sortierungen zu sortieren oder nach Filterkriterien zu filtern.
Vorgegeben Sortierungen sind auf- oder absteigend alphabetisch nach Namen des Rezepts, nach dem Schwierigkeitsgrad und nach dem Hinzufügedatum.
...
In der Rezept-Ansicht kann der Nutzer die Portionsgröße anpassen und das Rezept favorisieren.
Mit dem Zurück-Button gelangt der Nutzer wieder auf der Haupt-Ansicht.\par


\paragraph{Abbildung 3.3}

Im Aktivitätsdiagramm "Verwaltung als Administrator" wir gezeigt, welche Möglichkeiten der Nutzer als Administrator einer Gruppe in der "Gruppen-Ansicht" hat.
Der Administrator muss in der Verwaltungs-Ansicht die Gruppe wählen, in der er Gruppen Administrator ist.
Der Administrator landet so in der Gruppen-Detail-Ansicht.
Durch klicken auf ein Rezept kann der Gruppen Administrator, eingeblendete Rezepte ausblenden und ausgeblendete Rezepte einblenden.
...
\begin{figure}[!htp]
    ...
    \end{adjustbox}
    Abbildung 3.3: Aktivitätsdiagramm: Verwaltung als Administrator
    \label{fig:A33}
\end{figure}
\newpage
...
\textbf{Anforderungen:} \ref{RM11}, \ref{RS8} \\
...
\textbf{Nutzernamen und Profilbild ändern $\langle$F30$\rangle$}\\
\textbf{Anwendungsfall:} Der Nutzer möchte seinen Nutzernamen und sein Profilbild ändern.\\
\textbf{Anforderungen:} \ref{RS10}, \ref{RC4}\\
\textbf{Ziel:} Der Nutzer kann seinen Nutzernamen und sein Profilbild an seine Wünsche anpassen.\\
\textbf{Vorbedingung:} -\\
...
\textbf{Rezept suchen, sortieren, filtern, anschauen und favorisieren $\langle$F50$\rangle$}\\
...
\end{enumerate}
\textbf{Alternativen:} Der Nutzer kann die voreingestellte Portionsgröße auf die gewünschte Anzahl ändern. Des Weiter kann ein Rezept auch in der Form eines PDFs exportiert werden.
\newpage


\textbf{Gruppe erstellen und Gruppe auflösen $\langle$F60$\rangle$}\\
...
\textbf{Gruppe beitreten und verlassen $\langle$F70$\rangle$}\\
...
\textbf{Gruppenkürzel ausgeben $\langle$F80$\rangle$}\\
...
\textbf{Administrator ernennen und Administrator Status entfernen $\langle$F90$\rangle$}\\
...
\textbf{Nutzer kicken und Nutzer bannen $\langle$F100$\rangle$}\\
...
\textbf{Rezept melden und verwalten für die Gruppen $\langle$F110$\rangle$}\\
...
\textbf{Gruppennamen ändern $\langle$F120$\rangle$}\\
