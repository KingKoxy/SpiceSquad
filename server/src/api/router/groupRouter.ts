...
import {GroupCreateSchema, GroupDeleteSchema, GroupGetAllSchema, GroupJoinSchema, GroupLeaveSchema, GroupUpdateSchema} from "../../schemas/groupSchema"
...
export default class GroupRouter extends abstractRouter {
  ...
  */
  protected setupRoutes(): void {
    this.router.post(
      "/",
      this.checkAuth, this.schemaValidator.checkSchema(GroupCreateSchema),
      this.Controller.groupPost.bind(this.Controller)
    );
    this.router.delete(
      "/:groupId",
      this.checkAuth, this.schemaValidator.checkSchema(GroupDeleteSchema),
      this.Controller.groupDelete.bind(this.Controller)
    );
    this.router.patch(
      "/",
      this.checkAuth, this.schemaValidator.checkSchema(GroupUpdateSchema),
      this.Controller.groupPatch.bind(this.Controller)
    );
    this.router.patch(
      "/join",
      this.checkAuth, this.schemaValidator.checkSchema(GroupJoinSchema),
      this.Controller.groupJoin.bind(this.Controller)
    );
    this.router.patch(
      "/leave",
      this.checkAuth, this.schemaValidator.checkSchema(GroupLeaveSchema),
      this.Controller.groupLeave.bind(this.Controller)
    );
    this.router.get(
      "/",
      this.checkAuth, this.schemaValidator.checkSchema(GroupGetAllSchema),
